...
class MultiAvatarBuilder:
    def __init__(self, seed, env=True):
        self.seed = str(seed)
        self.env = env
        super().__init__()
    ...
    def getFinal(self, themes, part, partV, theme):
        logger.debug(part, partV, theme)
        ...
        result = [f"#{color}" for color in result]
        ...
        if result is not None:
            i = 0
            while i < len(result):
                logger.debug("replace", result[i], colors[i])
    ...
    def randomizer(self, seed):
        ...
        sha256Hash = sha256(seed.encode("utf-8")).hexdigest()
...
def multiavatar(string, sansEnv, ver):
    ...
    # Test
    logger.debug(sP["14"]["mouth"])
    logger.debug(sP["05"]["head"])
